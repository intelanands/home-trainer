...
- **Auth**: the app shows a PIN lock screen on first launch per device (verified via GET /api/history; permissive when offline/unverifiable — the server still guards all data). The static files remain technically fetchable by URL (same content as the public repo); the history API is PIN-protected (user declined Cloudflare Access/Google as too much setup). PIN lives in `/opt/home-trainer-data/pin.txt` (chmod 600, generated by setup-gym.sh, read per-request so rotation = edit the file). Clients send it as `X-Gym-Pin`; the app asks once per device (banner → prompt) and stores it in localStorage `trainer.pin`. Brute force: global 30-fails/hour throttle + 0.5s delay per failure. If the user loses the PIN: `ssh administrator@192.168.1.13 "cat /opt/home-trainer-data/pin.txt"`. The SW/sync hardening (no caching of redirects/HTML on data paths, sync trusts only `{ok:true}`) stays — it also keeps the door open for Cloudflare Access later.

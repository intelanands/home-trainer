...
Updates are live immediately — there is NO service worker or client cache anymore; every load is fresh from the server (nginx sends `Cache-Control: no-cache` for app files). `.claude/settings.local.json` (gitignored) allows the ssh command without prompts.
...
- **Auth = demo mode** (July 2026, user wants to show the app around): the app, plan, and images are PUBLIC read-only; everything that matters is guarded by the history API (`gymtok` cookie or `X-Gym-Pin` header). Signed out, the app shows a "Demo mode" banner, hides History, and the workout player runs but doesn't save. `#btn-auth` (top-right) toggles Sign in / Sign out (POST `/api/logout` clears the cookie); the app learns its state from GET `/api/auth` at launch — offline defaults to signed-in so the owner is never locked out locally (the server still refuses unauthenticated data access). The earlier full nginx `auth_request` wall was deliberately removed for demo-ability — don't re-add it without asking the user. `login.html` → POST `/api/login` → 180-day HttpOnly cookie `gymtok` = sha256(pin:session-gen) — a token, never the PIN. **Sign out everywhere**: `ssh administrator@192.168.1.13 "date +%s%N > /opt/home-trainer-data/session-gen.txt"` (invalidates all cookies instantly; PIN unchanged; no restart). PIN lives in `/opt/home-trainer-data/pin.txt` (chmod 600, read per-request → rotation = edit the file, no restart). Brute force: wrong guesses get 0.5s delay, 30/hour lockout; absent-PIN requests don't count (so anonymous visits can't lock the user out) and a valid PIN is never throttled. Lost PIN: `ssh administrator@192.168.1.13 "cat /opt/home-trainer-data/pin.txt"`. Verification from outside: the app loads publicly; `/api/history` must 401 without credentials and 200 with `-H "X-Gym-Pin: <pin>"`.
...
- `js/history.js` — localStorage log (`trainer.history.v1`), export/copy, auto-sync to server (POST `./api/history`; entries carry `synced:false` until confirmed, retried on every app launch), and restore-from-server when local storage is empty (new phone / cleared data = lossless)
...
- `sw.js` — a KILL SWITCH, not a service worker (retired July 2026: offline-first caching caused every update/auth headache — stale versions, devices stuck behind the login wall). The file must keep existing so old registrations fetch it, wipe their caches, and unregister. **Do not add a caching service worker back.** `APP_VERSION` in `js/app.js` is the footer display; the `?v=` query on the css/js tags in `index.html` busts HTTP/proxy caches. Bump both together on notable changes. App files are served `Cache-Control: no-store`.

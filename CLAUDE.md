...
- `sw.js` — a KILL SWITCH, not a service worker (retired July 2026: offline-first caching caused every update/auth headache — stale versions, devices stuck behind the login wall). The file must keep existing so old registrations fetch it, wipe their caches, and unregister. **Do not add a caching service worker back.** `APP_VERSION` in `js/app.js` is cosmetic (footer display) — bump it on notable changes so the user can confirm what they're running; no other version discipline needed.
